...
// In-memory fallback queues if Redis not configured
const inMemoryQueues = new Map(); // filterKey => [socketId...]
...
async function pushWaiting(filterKey, socketId) {
  if (redisClient) {
    try {
      ...
      console.warn('Redis push waiting failed, using in-memory', e);
...
async function popWaiting(filterKey) {
  if (redisClient) {
    try {
      const v = await redisClient.lPop(`waiting:${filterKey}`);
      return v;
    } catch (e) {
      console.warn('Redis pop waiting failed, using in-memory', e);
      const arr = inMemoryQueues.get(filterKey) || [];
